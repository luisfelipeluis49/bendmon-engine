...
| RULESET_VERSION | canonical semantics identity / version | exact version identifier | `m3-1` for the first complete headless-battle contract; reserved M6 identity `m6-1`, activated only when M6 lands | all save/replay laws; cross-version reinterpretation forbidden |
