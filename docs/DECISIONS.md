...
- Approved completion boundary: After an action and all of its immediate aftermath commit atomically, evaluate completion before the next queued event. On a terminal result, commit the result exactly once and discard every pending event. Discarded actions never reach execution, so release reservations without starting cooldown or consuming action RNG/resources. The outcome when the same action leaves every side without a viable combatant remains the next decision.
...
- Approved baseline critical chance: An eligible damaging action that passed accuracy has a base critical chance of 625/10000 (6.25%) before engine-owned critical modifiers. Nondamaging actions do not make critical checks. Critical multiplier, maximum chance and exact draw-consumption rules remain separate decisions.
