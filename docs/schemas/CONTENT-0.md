...
Development schema, implemented for the owner's next-part request. This remains an inert content loading slice even though later battle, encounter and world structures are now approved. Fields outside this contract reject. The move schema admits only approved timing, accuracy and animation references; move effects and the item/effect/recipe/capacity-tier authoring surface await additive schemas under approved D13–D16 authority.
...
- `manifest.json`: `catalogs` (relative JSON paths), `maps` (relative JSON paths), `encounters` (relative JSON paths), optional `moves` (relative JSON paths), `assets` (asset records).
...
- Each move document: `id` (MoveId), `name`, exactly one of `accuracy` or `alwaysHit: true`, `windup`, `recovery`, `cooldown`, and `animation` (AssetId). Ordinary accuracy is 500..10000; `alwaysHit` bypasses the accuracy formula and draw. Windup is 0..600 ticks, recovery 30..600, and ordinary cooldown 60..3600. `animation` resolves to an asset in the manifest. Animation assets are presentation references only: animation completion does not schedule hits or advance logical time. The engine owns all timing and accuracy semantics; move documents only select the approved inputs.
