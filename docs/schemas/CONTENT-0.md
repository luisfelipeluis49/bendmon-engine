...
Development schema, implemented for the owner's next-part request. This remains an inert content loading slice even though later battle, encounter and world structures are now approved. Fields outside this contract reject. Move and mix descriptors admit only bounded timing, power, accuracy, component and animation metadata; no authored executable effects are accepted.
...
- `manifest.json`: `catalogs` (relative JSON paths), `maps` (relative JSON paths), `encounters` (relative JSON paths), optional `moves`, `mixRecipes` and `mixResults` (relative JSON paths), `assets` (asset records).
...
- Each move document: `id` (MoveId), `name`, exactly one of `accuracy` or `alwaysHit: true`, `windup`, `recovery`, `cooldown`, and `animation` (AssetId); optional `components` is 1..4 distinct engine TypeIds (0..9). Ordinary accuracy is 500..10000; `alwaysHit` bypasses the accuracy formula and draw. Windup is 0..600 ticks, recovery 30..600, cooldown 60..3600. `animation` resolves to an asset in the manifest. Animation assets are presentation references only: animation completion does not schedule hits or advance logical time.
- Each mix recipe document: `id` (RecipeId), `sourceA` and `sourceB` (distinct BaseMoveIds resolving to listed base move documents with exactly one declared component), and `result` (ResultId resolving to a declared mix result metadata descriptor). Source pairs are unordered and unique across the loaded recipe set; reversed duplicates reject. Result IDs use a separate host identity type and cannot textually collide with base move IDs.
- Each mix result document: `id` (ResultId), `name`, `components` (1..4 distinct engine TypeIds, each 0..9), `power` (1..200), exactly one of `accuracy` (500..10000) or `alwaysHit: true`, `windup` (0..600), `recovery` (30..600), `cooldown` (60..3600), and `animation` (AssetId resolving to an asset). This is metadata only; the host loader does not bind it to or validate a registered M4 effect program. No M4 program/effect serialization is accepted, and no code, callback, script, or arbitrary effect field is permitted.
...
`platform/content/loader.py` is imported as `content.loader` by adding the developer-owned absolute `platform/` directory to the shell/test Python module path; do not shadow Python's standard `platform` module. `load_project(path, kernel_path=None)` returns an immutable `LoadedProject` with canonical JSON bytes, SHA-256 content hash and typed read-only records, or raises `ContentError` holding stable sorted diagnostics. Kernel path override is developer/test API only, never read from content. Canonical identity covers validated project, parsed catalogs/maps/encounters, optional moves/mix recipes/mix results, and asset metadata+verified digests; object keys sorted, compact UTF-8, declaration containers sorted by typed ID, and unordered recipe sources canonicalized before hashing. `load_project` must invoke Bend before returning success. The current Bend kernel validates existing world/catalog references but not M6 recipes/results; the shell validates recipe/result metadata references, bounds, uniqueness, and unordered pair rules. It does not validate M4 program registration or bind descriptors to executable programs. No accepted cache supplied by a pack.
