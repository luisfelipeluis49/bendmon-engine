...
- Selected algorithm: xoshiro128** version `xoshiro128ss-1.1` with four U32 state words. Use only specified U32 wraparound, XOR, shifts, rotations and multiplication. Reject the all-zero state. Lock implementation to published/reference-derived known-answer vectors plus repository vectors across native and JavaScript backends. Bounded sampling uses rejection sampling and never modulo-biased reduction. The first complete M3 ruleset identity is `m3-1`.
